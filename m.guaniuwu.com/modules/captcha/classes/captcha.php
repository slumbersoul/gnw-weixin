...
abstract class Captcha
{
	...
	public function __construct($group = NULL)
	{
		...
		if ( ! is_array($config = captcha_config($group)))
		...
		if ($group !== 'default')
		{
			...
			if ( ! is_array($default = captcha_config('default')))
				throw new Kohana_Exception('Captcha group not defined in :group configuration',
					array(':group' => 'default'));
	...
	public function update_response_session()
	{
        ...
        //$msg = "START: ".$this->response."\t\n".  sha1(strtoupper($this->response))."\t\n". 
        //var_dump(Session::instance()->get('captcha_response'));die();
		//if( Request::$current){
            //$msg .= "\t\n ca: ". Request::current()->controller().' - ' . Request::current()->action();
        //}
        //$msg .= "\t\n\n";
        //error_log($msg,   3, '/tmp/abc.log');
	}
